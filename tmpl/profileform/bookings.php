...
if (count($rows) == 0) {
    ...
    $objTable->add_header('Group,Date,Event,Type,Spaces');
    $rows = $toolsHelper->getRows($sql);
    foreach ($rows as $row) {
        ...
        $objTable->add_item('<a href="' . $event_link . '">' . $row->title . '</a>');
...
echo $toolsHelper->showEvents($this->user->id, 'Y', $this->menu_id);

echo '<h2>Past Events organised by you</h2>';
//    Find past events
echo $toolsHelper->showEvents($this->user->id, 'N', $this->menu_id);
?>
